...
import { Folder, Star, GitBranch, ExternalLink, RefreshCw, AlertCircle } from 'lucide-react';
    ...
    const fetchData = async () => {
      try {
        ...
        if (!profileRes.ok) {
           ...
           if (profileRes.status === 403) throw new Error('Rate Limit Exceeded. Add token to .env');
  ...
  if (loading) {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center text-blue-400 space-y-4">
        ...
        <span className="text-xs font-mono animate-pulse">Connecting to GitHub...</span>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center text-red-400 space-y-4 p-6 text-center">
        ...
        <p className="text-xs font-mono bg-white/5 p-2 rounded border border-white/10">{error}</p>
        <button 
         ...
         <div className="flex items-center gap-4">
            ...
            <div>
               <h2 className="text-xl font-bold flex items-center gap-2">
                 ...
                 <span className="text-[10px] px-2 py-0.5 rounded-full bg-blue-500/20 text-blue-300 border border-blue-500/30">PRO</span>
               </h2>
               <p className="text-xs text-gray-400 mt-1 line-clamp-1">{profile.bio || "Full Stack Developer | AI Enthusiast"}</p>
               <div className="flex gap-4 mt-3 text-[10px] text-gray-300 font-mono">
                  <span className="flex items-center gap-1"><Folder size={10} /> {profile.public_repos} Repos</span>
                  <span className="flex items-center gap-1"><ExternalLink size={10} /> {profile.followers} Followers</span>
               </div>
            </div>
         </div>
      </div>
